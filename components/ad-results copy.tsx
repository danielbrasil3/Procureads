...
export function AdResults({ pageGroups, totalPages, isVIP }: AdResultsProps) {
  ...
  const [expandedAdBodies, setExpandedAdBodies] = useState<Set<string>>(new Set())
  ...
  const toggleAdBody = (adId: string) => {
    setExpandedAdBodies(prev => {
      const next = new Set(prev)
      if (next.has(adId)) {
        next.delete(adId)
      } else {
                  ...
                  {group.ads.map((ad) => (
                    <Card key={ad.id} className="flex flex-col">
                      ...
                      <CardContent className="flex-grow ">
                        <div className="mb-4">
                          <p className={`text-sm text-muted-foreground ${expandedAdBodies.has(ad.id) ? '' : 'line-clamp-5'}`}>
                            {ad.body}
                          </p>
                          {ad.body.length > 300 && (
                            <button
                              onClick={() => toggleAdBody(ad.id)}
                              className="text-sm text-primary hover:underline mt-1"
                            >
                              {expandedAdBodies.has(ad.id) ? 'Ver menos' : 'Ver mais'}
                            </button>
                          )}
                        </div>
